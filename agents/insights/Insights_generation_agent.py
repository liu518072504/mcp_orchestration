...
class InsightCardFilter:
    ...
    async def filter(self, new_cards: List[InsightCard], existing_cards: List[InsightCard], schema: Dict[str, Any]) -> List[InsightCard]:
    ...
    def filter_schema_relevance(self, cards: List[InsightCard], schema: Dict[str, Any], threshold: float = 0.1) -> List[InsightCard]:
    ...
    def filter_duplicates(self, new_cards: List[InsightCard], existing_cards: List[InsightCard], threshold: float = 0.85) -> List[InsightCard]:
...
class PromptBuilder:
    ...
    def build_prompt(self, schema: Dict[str, Any], stats: str, previous_cards: List[InsightCard], num_questions: int, num_examples: int = 6) -> str:
...
class SQLAgent:
            ...
            async with aiosqlite.connect(db_path) as db:
                for i, (question, breakdown, measure) in enumerate(zip(questions, breakdowns, measures), 1):
                    ...
                    try:
                        ...
                        sql_result = await self._question_to_sql(question, breakdown, measure, table_info, table_name, db)
                        if not sql_result["success"]:
    ...
    def _build_sql_prompt(self, question: str, breakdown: str, measure: str, table_info: Dict[str, Any], table_name: str, error_history: List[Dict]) -> str:
...
class ReflexionSQLAgent(SQLAgent):
    def __init__(self, claude_client, max_retries=5):
